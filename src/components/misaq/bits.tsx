...
export function CompatibilityRing({
  ...
  return (
    <div className="relative inline-flex" style={{ width: size, height: size }}>
      ...
      <span
        className={cn(
          "absolute inset-0 flex items-center justify-center font-display text-sm font-semibold",
...
export function VerifiedBadge({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      <svg viewBox="0 0 24 24" className="h-3 w-3 fill-primary">
        <path d="M12 2l2.09 2.26L17 4l1 2.83 2.83 1L20 10.5l1.83 2.83L19 15l-1 2.83-2.91-.26L12 20l-2.09-2.43L6 17.83 5 15l-2.83-1.67L4 10.5 2.17 7.83 5 6l1-2.83 2.91.26z" />
      </svg>
      {t("profile.verified")}
    </span>
  );
...
import { useMe, useChats, useProposals, hasPhotoAccess, type Person } from "@/lib/mock";
...
export function Avatar({
  ...
  person: { id?: string; photo?: string; avatar: string; name: string; photoPrivacy?: string };
  size?: number;
}) {
  const [me] = useMe();
  ...
  const context = path.includes("/admin") ? "admin" : path.includes("/wali") ? "wali" : "member";

  const hasAccess = hasPhotoAccess(me, person as unknown as Person, chats, proposals, context);
  const isBlurred = showImage && !hasAccess;
      ...
      {showImage ? (
        <img
          ...
          className={cn(
            ...
            isBlurred && "filter blur-md scale-125",
          )}
        />
      ) : (
        initials
...
export function PhotoBg({
  ...
  const hasAccess = hasPhotoAccess(me, person as unknown as Person, chats, proposals, context);
  const isBlurred = (person.photo && !err && !hasAccess) || propIsBlurred;

  ...
  return (
    <div className="p-4 pb-8 space-y-2">
      {calls.map((c, i) => (
        ...
        >
          <span
            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${c.type === "video" ? "bg-primary/10 text-primary" : "bg-gold/20 text-[color:var(--color-gold-foreground)]"}`}
          >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-[10px] text-muted-foreground">
              {c.type === "video" ? t("call.video") : t("call.voice")} · {c.dur} · {c.when}
            </p>
          </div>
          {c.wali ? (
            <span className="shrink-0 text-[10px] text-primary">
              {t("admin.calls.waliPresent")}
            </span>
          ) : (

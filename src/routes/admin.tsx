...
function AdminShell() {
  ...
  useEffect(() => {
    if (path.startsWith("/admin") && !isLogin && !isAdminAuthed()) {
  ...
  return (
    <PhoneFrame>
      <div className="flex min-h-full flex-col">
        ...
        <nav
          className="scrollbar-none flex snap-x snap-mandatory gap-1.5 overflow-x-auto border-b border-border bg-surface px-3 py-2"
          ...
          {nav.map((n) => {
            ...
            return (
              <Link
                ...
                to={n.to as "/admin"}
                className={cn(
                  "flex shrink-0 snap-start items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-medium transition-colors",
                  active

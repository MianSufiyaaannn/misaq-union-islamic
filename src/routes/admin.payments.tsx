      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card">
        <ul className="divide-y divide-border">
          {rows.map((r, i) => (
            <li
              key={i}
              className="grid grid-cols-[minmax(0,1fr)_auto_auto] gap-3 px-3 py-2.5 text-sm"
            >
              ...
              <span
                className={`shrink-0 self-center rounded-full px-2 py-0.5 text-[10px] ${r.statusKey === "success" ? "bg-primary/10 text-primary" : r.statusKey === "refund" ? "bg-gold/20 text-[color:var(--color-gold-foreground)]" : "bg-destructive/15 text-destructive"}`}
              >

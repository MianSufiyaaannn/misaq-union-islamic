      ...
      <ul className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
        {pages.map((p) => (
          <li key={p.tk} className="flex items-center gap-3 px-4 py-3">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary"><FileText className="h-4 w-4" /></span>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium">{t(p.tk)}</p>
              <p className="truncate text-[10px] text-muted-foreground">{p.u}</p>
            </div>
            <button aria-label={t("common.edit")} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-muted"><Edit3 className="h-3.5 w-3.5" /></button>
          </li>
        ))}
      </ul>
    </div>

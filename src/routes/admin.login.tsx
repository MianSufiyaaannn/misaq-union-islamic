...
function AdminLogin() {
  return (
    <PhoneFrame>
      <TopBar title="Administrator" subtitle="Restricted access" />
      <div className="flex flex-1 flex-col px-6 pb-10 pt-4">
        <div className="flex items-center gap-3 rounded-2xl border border-gold/40 bg-gold/10 p-4 text-[color:var(--color-gold-foreground)]">
          ...
          <p className="text-xs">Admin accounts are pre-created. No public registration.</p>
        </div>

        <form className="mt-8 space-y-4" onSubmit={(e) => e.preventDefault()}>
          <Field label="Admin email" icon={<Mail className="h-4 w-4" />} type="email" placeholder="admin@misaq.app" />
          <Field label="Password" icon={<Lock className="h-4 w-4" />} type="password" placeholder="••••••••" />
          <Field label="Two-factor code" icon={<ShieldAlert className="h-4 w-4" />} placeholder="6-digit code" />
        ...
        <Link to="/auth/login" className="mt-auto pt-10 text-center text-xs text-muted-foreground">Back to member login</Link>
      </div>
    </PhoneFrame>
  );
}
